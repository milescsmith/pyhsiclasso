...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            [0,0],
            None,
            marks=pytest.mark.xfail(raises=UnboundLocalError)
        ),
        (
            "csv_data.csv",
            5,
...
def test_classification(hsic_obj: HSICLasso, load_input_data: Callable[..., Any], input_data: Literal['csv_data.csv', 'matlab_data.mat'] | None, num_feat: int | None, b_divisor: int | None, m: int| None, discrete_x: bool, load_covars: list[int], expected_A: list[int] | None):
    if not input_data:
...
def test_classification_non_divisor_block_size(hsic_obj: HSICLasso, load_input_data: Callable[..., Any], input_data: Literal['csv_data.csv'], num_feat: int, b_divisor: int, m: int, load_covars: list[int], expected_A: list[int]):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert str(w[-1].message) == f"B {B} must be an exact divisor of the number of samples {n}. Number of blocks {numblocks} will be approximated to {int(numblocks)}."
